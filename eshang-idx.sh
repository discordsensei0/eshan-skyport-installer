...
EOF
            ...
            # Check if running in restricted environment (like Firebase Studio)
            if [[ -f /opt/google/firebase-studio/environment ]]; then
                warning "Detected Firebase Studio environment!"
                info "Some features may be limited. Attempting to continue...\n"
            fi

            if command -v curl >/dev/null 2>&1; then
                # Try multiple URLs in case one fails
                if bash <(curl -s https://raw.githubusercontent.com/discordsensei0/eshan-skyport-installer/main/main.sh) 2>/dev/null; then
                    success "Cloud VM Manager session completed successfully."
                elif bash <(curl -s https://raw.githubusercontent.com/discordsensei0/eshan-skyport-installer/main/v2.sh) 2>/dev/null; then
                    success "Cloud VM Manager session completed successfully (v2)."
                else
                    error "Failed to fetch or execute the script."
                    warning "Check your internet connection or GitHub status."
                    echo -e "\n${YELLOW}You can manually run:${RESET}"
                    echo -e "  ${WHITE}bash <(curl -s https://raw.githubusercontent.com/discordsensei0/eshan-skyport-installer/main/main.sh)${RESET}"
                fi
            else
